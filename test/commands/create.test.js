...
describe('create', () => {
  it('creates a project from the template', async () => {
    ...
    expect(await pathExists(path.join(targetDir, 'CMakeLists.txt'))).to.equal(true)
    expect(await pathExists(path.join(targetDir, 'Makefile'))).to.equal(true)

    const cmake = await fs.readFile(path.join(targetDir, 'CMakeLists.txt'), 'utf8')
    expect(cmake).to.contain('project(MyGame)')

    const renamed = await pathExists(path.join(targetDir, 'src', 'MyGame_main.cpp'))
